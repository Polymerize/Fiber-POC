...
def analyze_fiber_stats(path, dist_map):
    ...
    # 1. Length (Geodesic)
    length = sum(math.hypot(path[i][1]-path[i-1][1], path[i][0]-path[i-1][0]) for i in range(1, len(path)))
    ...
    # 3. Curvature (Tortuosity)
    # Tortuosity = Geodesic Length / Euclidean Distance (End-to-End)
    euclidean_dist = math.hypot(path[-1][1]-path[0][1], path[-1][0]-path[0][0])
    tortuosity = length / euclidean_dist if euclidean_dist > 0 else 1.0
...
if uploaded_file is not None:
    with st.spinner('Analyzing fibers...'):
        ...
        skeleton_final, fibers, img_original, dist_transform = process_fiber_image(uploaded_file.read())
        ...
        for i, path in enumerate(fibers):
            ...
            if stats["length_px"] < min_length: continue
            stats["fiber_id"] = i + 1
            fiber_data.append(stats)
            ys, xs = [p[0] for p in path], [p[1] for p in path]
            cv2.rectangle(analysis_vis, (min(xs)-3, min(ys)-3), (max(xs)+3, max(ys)+3), (0, 255, 0), 1)
            cv2.putText(analysis_vis, str(i+1), (min(xs), min(ys)-5), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 255, 255), 1)

        df = pd.DataFrame(fiber_data)
        
        col1, col2 = st.columns([2, 1])
        with col1:
            st.image(analysis_vis, use_container_width=True, caption="Detection Visualization")
        with col2:
            if not df.empty:
                summary = {
                    "Total Fibers": len(df),
                    ...
                    "Avg Tortuosity": f"{df['tortuosity'].mean():.3f}"
                }
                st.subheader("Detailed Metrics")
                st.dataframe(df[["fiber_id", "length_px", "width_px", "tortuosity", "aspect_ratio"]], use_container_width=True)
